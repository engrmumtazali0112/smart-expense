...
def dashboard(request: Request, db: Session = Depends(get_db),
    ...
    users_query = get_accessible_users_query(current_user, db)
    user_ids = [u.id for u in users_query.all()]
    
    # Stats based on role permissions
    total_invoices = db.query(Invoice).filter(Invoice.user_id.in_(user_ids)).count()
    total_expenses = db.query(Expense).filter(Expense.user_id.in_(user_ids)).count()
    
    invoices = db.query(Invoice).filter(Invoice.user_id.in_(user_ids)).all()
    invoice_total = sum(inv.total for inv in invoices)
    
    expense_total = db.query(func.sum(Expense.amount)).filter(
        Expense.user_id.in_(user_ids)).scalar() or 0
    
    recent_invoices = db.query(Invoice).filter(Invoice.user_id.in_(user_ids))\
        .order_by(Invoice.created_at.desc()).limit(5).all()
    recent_expenses = db.query(Expense).filter(Expense.user_id.in_(user_ids))\
